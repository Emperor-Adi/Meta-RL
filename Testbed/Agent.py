...
class Agent:
    ...
    def train_network(self):
        if not self.memory.GAE_CALCULATED_Q:
            self.make_gae()
        states,actions,gae_r = self.memory.get_batch(self.TRAINING_BATCH_SIZE)
        ...
        batch_old_prediction = keras.utils.normalize(batch_old_prediction)
    ...
    def get_old_prediction(self, states):
        ...
        for state in states:
            state = np.reshape(state, (-1, self.state_dim[0]))
            return_batch.append(self.actor_old_network.predict_on_batch([state,self.dummy_advantage, self.dummy_old_prediciton])[0])
