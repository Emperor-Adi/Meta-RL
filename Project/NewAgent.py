...
class Agent:
    ...
    def train_network(self):
        if not self.memory.GAE_CALCULATED_Q:
            self.make_gae()
        states,actions,_,gae_rewards,_,_ = self.memory.get_batch(self.BATCH_SIZE)

        batch_s = np.vstack(states)
        batch_a = np.ndarray(np.vstack(actions))
        ...
        batch_a_f = np.zeros(shape=(len(batch_a),self.action_n))
        batch_a_f[:, batch_a.flatten()] = 1